...
//Executes an Ajax Request and return the result to the function.
function ajaxRequest(controllerName, actionName, data, callbackFunction) {
    $.ajax({
        type: "POST",
        url: String.format("/{0}/{1}/default.aspx", controllerName, actionName),
        data: data,
        contentType: "application/json",
        dataType: "json",
        success: function(data) { if (callbackFunction != null) callbackFunction(data) }
    });
}

//Appends overlay to a control. Optionally, we can pass a function for onBeforeLoadFunction or onLoadFunction events.
function appendOverlay(overlaySelector, targetSelector, onBeforeLoadFunction, onLoadFunction) {
    return $(overlaySelector).overlay({
        expose: {
            color: '#333',
            loadSpeed: 200,
            opacity: 0.3
        ...
        onBeforeLoad: function(data) {
            if (onBeforeLoadFunction != null)
                onBeforeLoadFunction(data.target, this)
        },
        onLoad: function(data) {
            if (onLoadFunction != null)
                onLoadFunction(data.target);
        }
    });
}

//Show a popup modal message in the center of screen.
...
function appendMessage(jqObj, msg, type) {
    ...
    switch (type) {
        case "error":
            css = "appendMessage appendError";
            src = "/static/img/error.jpg";
            break;

        case "success":
            css = "appendMessage appendSuccess";
            src = "/static/img/icon-ok.gif";
            break;

        case "warning":
            css = "appendMessage appendWarning";
            src = "";
            break;

        case "loading":
            css = "appendMessage appendLoading";
            src = "/static/img/ajax-loader-small.gif"
            break;
        case "info":
            css = "appendInfo appendSuccess";
            src = "/static/img/icon-ok.gif";
            break; 
    }

    var divMsg = "";
    divMsg = "<div class='" + css + "'>";
    divMsg += "<img src='" + src + "' />";
...
//Track every validation error during the Build your Closet process
function trackValidationError(message) {
    var encoded = $.toJSON(message);
    ajaxRequest("BuildYourCloset", "TrackValidationError", encoded, null); 
}
