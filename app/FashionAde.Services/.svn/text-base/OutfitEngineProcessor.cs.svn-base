...
namespace FashionAde.Services
{
    public class OutfitEngineProcessor : IOutfitEngineProcessor
    {
        ...
        private readonly HashSet<Combination> flavorCombinations = new HashSet<Combination>();
        private readonly HashSet<PreOutfit> outfits = new HashSet<PreOutfit>();
        private FashionFlavor currentFlavor;

        private IList<FashionFlavor> currentFlavors;
        private IList<Garment> currentGarments;

        private string fileName;
        private IList<StyleRule> lstStyleRules;
        private HashSet<Garment> newGarments;

        private string SharePath
        {
            get { return ConfigurationManager.AppSettings["OutfitEngine_SharePath"]; }
        }

        private string LocalDatabasePath
        {
            get { return ConfigurationManager.AppSettings["OutfitEngine_DatabaseFilePath"]; }
        }
        ...
        private readonly IIndexCreationService _outfitIndexingService;
        private readonly IClosetRepository _closetRepository;
        private readonly IOutfitUpdaterService _outfitUpdaterService;
        private readonly IStyleRuleRepository _styleRuleRepository;

        private static readonly ILog logger = LogManager.GetLogger(typeof(OutfitEngineProcessor).Namespace);
        ...
        public const int MAX_COMBINATIONS_PER_LAYER = 500;
        ...
        public IEnumerable<Combination> Combine(int minAmount, IEnumerable<Garment> garmentListFor1,
                                                IEnumerable<Garment> garmentListFor2,
                                                IEnumerable<Garment> garmentListFor3,
                                                IEnumerable<Garment> garmentListFor4,
                                                IEnumerable<Garment> garmentListFor5)
        {
            return Combine(minAmount, garmentListFor1, garmentListFor2, garmentListFor3, garmentListFor4,
                           garmentListFor5, null, null, null);
        }
        ...
        public IEnumerable<Combination> Combine(int minAmount, IEnumerable<Garment> garmentListFor1,
                                                ...
                                                IEnumerable<Garment> garmentListFor4,
                                                IEnumerable<Garment> garmentListFor5,
                                                IEnumerable<Garment> garmentListFor6,
                                                IEnumerable<Garment> garmentListFor7,
                                                IEnumerable<Garment> garmentListFor8)
        {
                ...
                foreach (Garment gb in garmentListFor2)
                {
                    ...
                    foreach (Garment gc in garmentListFor3)
                    {
                        ...
                        foreach (Garment gd in garmentListFor4)
                        {
                            ...
                            {
                                yield return
                                    new Combination {GarmentA = ga, GarmentB = gb, GarmentC = gc, GarmentD = gd};
                                continue;
                            }
                            ...
                            foreach (Garment ge in garmentListFor5)
                            {
                                if (garmentListFor6 == null && minAmount == 5)
                                {
                                    yield return
                                        new Combination
                                            {GarmentA = ga, GarmentB = gb, GarmentC = gc, GarmentD = gd, GarmentE = ge};
                                    continue;
                                }
