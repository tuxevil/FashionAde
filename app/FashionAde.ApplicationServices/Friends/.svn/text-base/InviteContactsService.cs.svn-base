...
namespace FashionAde.ApplicationServices
{
    ...
    public class InviteContactsService : IInviteContactsService
    {
        ...
        private IInvitationCodeGeneratorService invitationCodeGeneratorService;

        public InviteContactsService(IInvitationCodeRepository invitationCodeRepository, IBasicUserRepository registeredUserRepository, IFriendProviderRepository friendProviderRepository, IFriendCreatorService friendCreatorService, IMessageSenderService messageSenderService, IInvitationCodeGeneratorService invitationCodeGeneratorService)
        {
            ...
            this.invitationCodeGeneratorService = invitationCodeGeneratorService;
        }
        ...
        public void InviteUsers(BasicUser invitedBy, string comments, FriendProvider friendProvider, IList<IBasicUser> contacts, bool includeInvitationCode)
        {
            ...
            foreach (IBasicUser contact in contacts)
            {
                ...
                string confirmUrl = string.Format("/Friend/FriendsRequest/default.aspx");
                ...
                if (f.User is InvitedUser)
                {
                    if (includeInvitationCode)
                    {
                        ...
                        invitationCodeGeneratorService.Generate(f.User.EmailAddress, f.BasicUser, inviteData.InvitationCode);
