...
namespace FashionAde.ApplicationServices.SearchEngine
{
    ...
    public class SearchEngineService : IDisposable
    {
        ...
        private const string IsUpToDate = "IsUpToDate";
        ...
        private SearchEngineResponse PerformQuery(ICollection<SearchEngineResult> list, Query queryOrig, int start, int max, int blogId, int entryId, SearchParameters parameters, string sortField, bool isDescending)
        {
            ...
            if (parameters != null)
            {
                ...
                if (parameters.IsUpToDate)
                    query.Add(new TermQuery(new Term(IsUpToDate, Convert.ToInt32(parameters.IsUpToDate).ToString())), BooleanClause.Occur.MUST);
                ...
                if (parameters.EventTypes != null && parameters.EventTypes.Length > 0)
                {
                    QueryParser parser = new QueryParser(Lucene.Net.Util.Version.LUCENE_29, EventTypes, _analyzer);
                    parser.SetDefaultOperator(QueryParser.Operator.AND);

                    string joined = string.Join(" ", parameters.EventTypes.Select(x => "\"" + x + "\"").ToArray());
                    query.Add(parser.Parse(joined), BooleanClause.Occur.MUST);
                }
