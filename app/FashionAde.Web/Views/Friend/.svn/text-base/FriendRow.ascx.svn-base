    ...
    <td style="text-align: left;">
        <span style="font-weight: bold; margin-right: 10px; color: Black;" >
            <% if(!string.IsNullOrEmpty(Model.User.FirstName)){%>
                <%= HttpUtility.HtmlEncode(Model.User.FirstName) %>
            <%}%>
                           
            <% if (!string.IsNullOrEmpty(Model.User.LastName)) {%>
                <%= HttpUtility.HtmlEncode(Model.User.LastName) %>
            <%}%>
        </span>
    </td>
    <td style="text-align: left;"><span style="font-weight: normal; color: Black;"><%= HttpUtility.HtmlEncode(Model.User.EmailAddress) %></span></td>
    <td><% = Model.Status %></td>
