...
<tr style="background-color:#EDEDED; border: solid 10px white; ">
    <td style="text-align: left;">
        <span style="font-weight: bold; margin-right: 10px; color: Black;" >
            <% if(!string.IsNullOrEmpty(Model.BasicUser.FirstName)){%>
                <%= HttpUtility.HtmlEncode(Model.BasicUser.FirstName) %>
            <% } %>
            
            <% if (!string.IsNullOrEmpty(Model.BasicUser.LastName)) { %>
                <%= HttpUtility.HtmlEncode(Model.BasicUser.LastName) %>
            <% } %>            
        </span>
    </td>
